...
def get_segment_to_point_dist(start, end, points):
    ...
    cross_product = torch_2d_cross(line_vec, pnt_vec)
    sign = torch.sign(cross_product) # if zero is orthogonal, otherwise it represents the side of the projection
...
def get_gap(points, racing_line, calculate_distance=False, cum_distance_in_racing_line=None):
    ...
    if calculate_distance:
        closest_segments = projection_len_s1.clone()
        cum_dist_to_rl_point = cum_distance_in_racing_line[rl_point].clone()
    ...
    dist_min = torch.argmin(torch.cat((dist_s0.reshape(-1,1) , dist_s1.reshape(-1,1) ) ,dim=1), axis=1)
